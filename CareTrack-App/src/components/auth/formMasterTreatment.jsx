...
export default function FormMasterTreatment({ formType, addMethod, updateMethod, selectedData }) {
    ...
    const [formTyping, setFormTyping] = useState({ nameTyping: false, codeTyping: false, costTyping: false });
    ...
    const validateForm = () => {
        ...
        let cost = String(form.cost);

        if (!cost.trim()) {
            newErrors.cost = "Treatment cost is required!";
        } else if (Number(cost.trim()) < 1) {
            newErrors.cost = "Must be more than 0!";
        }
    ...
    return (
        <form onSubmit={formType === 'add' ? handleAddTreatment : handleUpdateTreatment} >
            <div className="pb-1">
            ...
            <div className="pb-1">
                <label htmlFor="cost" className="text-black/90 text-sm">
            ...
            <div className="relative pb-4">
                <input
                    ...
                    className={`focus:bg-blue-100 focus:outline-none p-4 border-solid rounded-lg h-8 w-full  ${errors.cost ? "border-red-500" : "border-slate-800/10"
                        }`}
                    style={{ borderWidth: 1 }}
                />
                {errors.cost && (
                    <span className="text-[0.70rem] text-red-500">{errors.cost}</span>
                )}
            </div>
