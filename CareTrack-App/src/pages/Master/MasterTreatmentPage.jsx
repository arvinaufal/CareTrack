...
import { FaDatabase, FaHandHoldingMedical, FaMagnifyingGlass } from "react-icons/fa6";
import DataTable from "react-data-table-component";
import axios from "axios"; // Import Axios untuk AJAX
import FormMasterTreatment from "../../components/auth/formMasterTreatment";
import { Report } from "notiflix";

export default function MasterTreatmentPage() {
    const [selectedData, setSelectedData] = useState({ id: '', name: '', cost: 0, code: '' });
    ...
    const getTreatment = async () => {
        ...
        const endpoint = `http://127.0.0.1:3000/api/master/treatments?search=${searchKeyword}&perPage=${perPage}&page=${page}`;
        ...
        try {
            ...
            setDataTreatment(response.data.data);
    ...
    const addTreatment = async (payload) => {
        const { name, code, cost } = payload;

        try {
            const response = await axios.post(
                `http://127.0.0.1:3000/api/master/treatments`,
                {
                    name, code, cost
                },
                {
    ...
    const updateTreatment = async (payload) => {
        const { id, name, code, cost } = payload;
        try {
            const response = await axios.put(
                `http://127.0.0.1:3000/api/master/treatments/${id}`,
                {
                    name,
                    code,
                    cost,
    ...
    const handleModalUpdate = (data) => {
        const { id, name, code, cost } = data;
        setSelectedData({ id, name, cost, code });
        ...
        <section className="w-full flex flex-row min-h-screen">
            ...
            <div className="flex flex-col w-5/6 bg-slate-100 px-6 py-4 gap-4">
                <div className="flex flex-row w-full rounded-xl shadow-xl bg-white px-4 py-4">
                    <div className="flex w-1/2 items-center content-center gap-4 flex-row">
                        <FaHandHoldingMedical size={40} className={`text-[#fc84ac]`} />
                        <span className="text-xl font-semibold text-[#fc84ac]">Data Master - Treatment</span>
                    </div>
                    <div className="flex flex-row w-1/2 px-4 justify-end">
                        <div className="breadcrumbs text-md">
                            <ul>
                                ...
                                <li>
                                    <a>
                                        <FaHandHoldingMedical size={20} className={`text-[#fc84ac]`} />
                                        <span className="text-[#fc84ac]">
                                            Treatment
                                        </span>
                                    </a>
            ...
            <dialog id="modal-add" className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    <h3 className="font-bold text-lg">Add Treatment</h3>
                    <div className="modal-action">
                        <div className="flex flex-col w-full">
                            <FormMasterTreatment formType="add" addMethod={addTreatment} />
                        </div>
                    </div>
                </div>
            </dialog>
            ...
            <dialog id="modal-update" className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    <h3 className="font-bold text-lg">Update Treatment</h3>
                    <div className="modal-action">
                        <div className="flex flex-col w-full">
                            <FormMasterTreatment formType="update" updateMethod={updateTreatment} selectedData={selectedData} />
                        </div>
                    </div>
                </div>
            </dialog>
            ...
            <dialog id="modal-delete" className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    <h3 className="font-bold text-lg">Delete Treatment</h3>
                    <div className="modal-action">
                        <div className="flex flex-col w-full gap-4">
                            <div className="w-full flex ">
                                Are you sure you want you want to delete treatment "{selectedData.name}"?
                            </div>
                            <div className="flex w-full mt-4 justify-end gap-4">
                                ...
                                {pending ? (
                                    ...
                                    <div
                                        onClick={() => deleteTreatment(selectedData.id)}
